...
public:
    ...
    void SetInTrade(bool b = true) { _isInTrade = b; }
    bool IsInTrade() const { return _isInTrade; }
    ...
    Bag* GetContainer() { return _container; }
    uint8 GetBagSlot() const;
    void SetSlot(uint8 slot) { _slot = slot; }
    uint16 GetPos() const { return uint16(GetBagSlot()) << 8 | GetSlot(); }
    void SetContainer(Bag* container) { _container = container; }

    bool IsInBag() const { return _container != NULL; }
    ...
    ItemRandomEnchantmentId GetItemRandomEnchantmentId() const { return _randomEnchantment; }
    ...
    Loot loot;
    bool _isLootGenerated;

    // Update States
    ItemUpdateState GetState() const { return _updateState; }
    void SetState(ItemUpdateState state, Player* forplayer = NULL);
    void RemoveFromUpdateQueueOf(Player* player);
    bool IsInUpdateQueue() const { return _queuePos != -1; }
    uint16 GetQueuePos() const { return _queuePos; }
    ...
    // Item Refund system -- means `return of goods`
    void SetNotRefundable(Player* owner, bool changestate = true, SQLTransaction* trans = nullptr, bool addToCollection = true);
    void SetRefundRecipient(ObjectGuid const& guid) { _refundRecipient = guid; }
    void SetPaidMoney(uint64 money) { _paidMoney = money; }
    void SetPaidExtendedCost(uint32 iece) { _paidExtendedCost = iece; }

    ObjectGuid const& GetRefundRecipient() const { return _refundRecipient; }
    uint64 GetPaidMoney() const { return _paidMoney; }
    uint32 GetPaidExtendedCost() const { return _paidExtendedCost; }
    ...
    ObjectGuid GetChildItem() const { return _childItem; }
    void SetChildItem(ObjectGuid childItem) { _childItem = childItem; }
...
private:
    ...
    bool _isInTrade;
    time_t _lastPlayedTimeUpdate;
    ObjectGuid _refundRecipient;
    uint64 _paidMoney;
    uint32 _paidExtendedCost;
    GuidSet _allowedGUIDs;
    // store separately to easily find which bonus list is the one randomly given for stat rerolling
    ItemRandomEnchantmentId _randomEnchantment;
    ObjectGuid _childItem;
    std::unordered_map<uint32, uint16> _artifactPowerIdToIndex;
    std::array<uint32, MAX_ITEM_PROTO_SOCKETS> _gemScalingLevels;
};
#endif
