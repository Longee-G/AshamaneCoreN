...
bool AreaTrigger::Create(uint32 spellMiscId, Unit* caster, Unit* target, SpellInfo const* spell, Position const& pos
    , int32 duration, uint32 spellXSpellVisualId, ObjectGuid const& castId, AuraEffect const* aurEff)
{
    ...
    {
        TC_LOG_ERROR("entities.areatrigger", "AreaTrigger (spellMiscId %u) not created. Invalid areatrigger miscid (%u) from spellid (%u)",
            spellMiscId, spellMiscId, spell ? spell->Id : 0);
        return false;
    }
...
// this AreaTrigger defines in table `areatrigger_template`
