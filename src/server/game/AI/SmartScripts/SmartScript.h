...
public:
    ...
    Creature* GetCreatureInDist(float dist, uint32 entry);
    GameObject* GetGameObjectInDist(float dist, uint32 entry);
...
private:
    ...
    // use for `SMART_EVENT_TEXT_OVER`
    uint32 _talkerEntry;    // Talker entry
    uint32 _lastTextID;     // last text of talker    
    uint32 _textTimer;      // Talk timer, when decreases to 0, trigger Event
    bool _isTextTimerUsed;
    uint32 _nextTalker;     // creature or gameobject db guid
