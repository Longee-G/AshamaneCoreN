            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                for (std::list<Unit*>::const_iterator iter = Ghouls.begin(); iter != Ghouls.end(); ++iter)
                {
                    ...
                    if (Creature* sylvanas = instance->GetCreature(NPC_ECHO_OF_SYLVANAS))
                    {
                        ...
                        // If the agle is minor than 90?                        // (*iter) is the right ghoul for which we could check if player is in between
                        if (acos(cosAngle) < 1.57f && !nextGhoul1)
                        {
                            nextGhoul1 = (*iter);
                        }

                        // If the agle is major than 270?                        // (*iter) is the right ghoul for which we could check if player is in between
